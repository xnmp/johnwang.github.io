...
proper price, and if one wishes to buy or sell they can at a price
...
When Google finance says that a stock is trading at $10, what's really
happening is that someone is willing to buy at \$9.99 and someone
is willing to sell at \$10.01, and we just take the average of the
two prices to get the fair price. As we soon see, this isn't always
a very good idea. 

A little big of jargon before we continue:

* The \emph{bid} is the price at which someone is willing to buy the
asset in question.
* The \emph{offer} is the price at which someone is willing to sell
the asset.
* The \emph{midpoint} is the average of the bid and the offer.
* The \emph{spread} is the difference between the bid and the offer. 
...
This leads to the \emph{inversed weighted midpoint} (IWM), where we
...
the midpoint, the imbalance in the order book, whether it\textquoteright s
...
I found somewhat high frequency market data from the website http://hopey.netfonds.no/market.php.
...
alternative is the NYSE OpenBook which may be found at. 
...
and the batch size. I used a Nadam optimizer (basically RMSProp with
...
the evaluation metrics over the entire test set, where we have noticeable
...
The gray shaded region represents the bid and offer depth. What's
...
Nevertheless what we \emph{can} do is pit the models against each
...
one can come up with an \emph{optimal} trading strategy, in the one
